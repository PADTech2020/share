...
class ProjectsController extends BaseController
{
    ...
    public function store(ProjectsRequest $request, BaseHttpResponse $response)
    {
        ...
        $string = str_replace(' ', '-', $projects->name); // Replaces all spaces with hyphens.

        $projects->slug = preg_replace('/[^\x{0600}-\x{06FF}A-Za-z\- %^&*()]/u', '', $string);//preg_replace('/[^A-Za-z0-9\-]/', '', $string);
